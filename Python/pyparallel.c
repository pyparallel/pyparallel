...
)
{
    ...
    if (parent) {
        ...
        /*
        if (0 && PxSocket_ODBC(parent)) {
            if (!parent->henv)
                // Should be initialized in InitProtocol().
                __debugbreak();
...
_PyAsync_ModInit(void)
{
    ...
    //PxOdbcModule_APIObject *pxodbc_api;
