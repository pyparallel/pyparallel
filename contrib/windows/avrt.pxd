...
cdef extern from *:
    ...
    BOOL __stdcall AvSetMmThreadPriority(
        HANDLE AvrtHandle,
        AVRT_PRIORITY Priority
    )

    HANDLE __stdcall AvSetMmThreadCharacteristics(
        LPCTSTR TaskName,
        LPDWORD TaskIndex
    )

    HANDLE __stdcall AvSetMmMaxThreadCharacteristics(
        LPCTSTR FirstTask,
        LPCTSTR SecondTask,
        LPDWORD TaskIndex
    )
    ...
    BOOL __stdcall AvQuerySystemResponsiveness(
        HANDLE AvrtHandle,
